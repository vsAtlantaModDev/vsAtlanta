...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'seabass':
			dialogue = dialogue = CoolUtil.coolTextFile(Paths.txt('seabass/seabassDialogue'));
			case 'sofishticated':
			dialogue = dialogue = CoolUtil.coolTextFile(Paths.txt('sofishticated/sofishticatedDialogue'));
			case 'finished':
			dialogue = dialogue = CoolUtil.coolTextFile(Paths.txt('finished/finishedDialogue'));
		}
		...
		switch(SONG.stage)
		{
			case 'seabass': 
				{		  
						defaultCamZoom = 0.8;
						curStage = 'seabass';
						var bg:FlxSprite = new FlxSprite(-520, -120).loadGraphic(Paths.image('sea_background'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
						
						var fg:FlxSprite = new FlxSprite(-500, 740).loadGraphic(Paths.image('sea_foreground'));
	...
	{
		if(curSong.toLowerCase() == 'seabass' || curSong.toLowerCase() == 'sofishticated' || curSong.toLowerCase() == 'finished'){
			dad.animation.stop();
			defaultCamZoom = 0.7;
		}
	...
	{
		if(FlxG.save.data.distractions){
			FlxG.sound.play(Paths.soundRandom('seabassass', 0, 1), 0.7);
